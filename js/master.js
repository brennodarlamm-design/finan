...
async function masterFetchWithTimeout(url, options = {}, timeoutMs = 20000) {
...
const MasterAdmin = {
  ...
  async carregarEmpresas(force = false) {
    ...
    try {
      const resp = await masterFetchWithTimeout('/api/admin?action=tenants', {
  ...
  async carregarCobrancas(force = false) {
    ...
    try {
      const resp = await masterFetchWithTimeout('/api/admin?action=billing', {
        headers:(typeof Auth !== 'undefined' && Auth.getAuthHeaders) ? Auth.getAuthHeaders() : {}
      });
  ...
  async carregarErrosSaaS(force = false) {
    ...
    try {
      const resp = await masterFetchWithTimeout('/api/admin?action=client_errors&limit=100', {
        headers:(typeof Auth !== 'undefined' && Auth.getAuthHeaders) ? Auth.getAuthHeaders() : {}
      });
      const data = await resp.json().catch(() => ({}));
      if (resp.ok && data.success) {
  ...
  async carregarIntegridade(force = false) {
    ...
    try {
      const resp = await masterFetchWithTimeout('/api/admin?action=integrity_status', { headers:(typeof Auth !== 'undefined' && Auth.getAuthHeaders) ? Auth.getAuthHeaders() : {} });
      const data = await resp.json().catch(() => ({}));
  ...
  async carregarContasBancarias(force = false) {
    ...
    try {
      const resp = await masterFetchWithTimeout('/api/admin?action=bank_accounts_overview', {
        headers: (typeof Auth !== 'undefined' && Auth.getAuthHeaders) ? Auth.getAuthHeaders() : {}
      });
      const data = await resp.json().catch(() => ({}));
      if (resp.ok && data.success) {
  ...
  async limparErrosAntigos() {
    ...
    try {
      const resp = await masterFetchWithTimeout('/api/admin?action=clear_old_client_errors', {
        method: 'POST',
  ...
  async confirmarPagamento(invoiceId) {
    ...
    try {
      const resp = await masterFetchWithTimeout('/api/admin?action=confirm_payment', {
        method:'POST',
        headers:(typeof Auth !== 'undefined' ? Auth.getAuthHeaders() : { 'Content-Type':'application/json' }),
  ...
  async simularWebhookPix(invoiceId) {
    ...
    try {
      const resp = await masterFetchWithTimeout('/api/admin?action=simulate_webhook_pix', {
        method: 'POST',
        headers: (typeof Auth !== 'undefined' ? Auth.getAuthHeaders() : { 'Content-Type': 'application/json' }),
        body: JSON.stringify({
          invoiceId: inv.id,
  ...
  async impersonarEmpresa(tenantId) {
    ...
    try {
      ...
      try {
        await masterFetchWithTimeout('/api/admin?action=support_start', {
          method: 'POST',
      ...
      const res = await masterFetchWithTimeout('/api/admin?action=impersonate', {
        method: 'POST',
        headers: (typeof Auth !== 'undefined' && Auth.getAuthHeaders) ? Auth.getAuthHeaders() : { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tenantId })
      });
  ...
  async salvarEdicaoEmpresa(e, tenantId) {
    ...
    try {
      const res = await masterFetchWithTimeout('/api/admin?action=update_tenant', {
        method: 'PATCH',
        headers: (typeof Auth !== 'undefined' && Auth.getAuthHeaders) ? Auth.getAuthHeaders() : { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          tenantId,
  ...
  async gerarChaveEmpresa(tenantIdEnc, tenantNomeEnc) {
    ...
    try {
      const res = await masterFetchWithTimeout('/api/admin?action=generate_tenant_access_key', {
        method: 'POST',
        headers: (typeof Auth !== 'undefined' && Auth.getAuthHeaders) ? Auth.getAuthHeaders() : { 'Content-Type': 'application/json' },
  ...
  async salvarNovaEmpresa(e) {
    ...
    try {
      const res = await masterFetchWithTimeout('/api/admin?action=create_tenant', {
        method: 'POST',
        headers: (typeof Auth !== 'undefined' && Auth.getAuthHeaders) ? Auth.getAuthHeaders() : { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          nome_fantasia: nome,
  ...
  async enviarNotificacaoCobranca() {
    ...
    try {
      ...
      const res = await masterFetchWithTimeout('/api/admin?action=send_billing_notice', {
        method: 'POST',
        headers,
        body: JSON.stringify({
          tenantId: emp.id,
  ...
  async executarVarreduraCobranca() {
    ...
    try {
      const headers = (typeof Auth !== 'undefined' && Auth.getAuthHeaders) ? Auth.getAuthHeaders() : { 'Content-Type': 'application/json' };
      const res = await masterFetchWithTimeout('/api/admin?action=trigger_billing_sweep', {
        method: 'POST',
        headers
      });
      const data = await res.json().catch(() => ({}));
  ...
  async regenerarBackupCodes() {
    ...
    try {
      ...
      const res = await masterFetchWithTimeout('/api/admin?action=mfa_regenerate_backup_codes', {
        method: 'POST',
        headers,
