...
const NFe = {
  ...
  _renderListaLancamentos(filtro, chave) {
    ...
    return filtrados.map(l => {
      ...
      return `
        <div onclick="NFe._confirmarAnexo('${l.id}','${chave}')"
  ...
  _filtrarLancamentos(filtro, chave) {
    ...
    if (el) el.innerHTML = this._renderListaLancamentos(filtro, chave);
  },
  ...
  async _processarArquivosXML(files) {
    ...
    for (const file of xmlFiles) {
      try {
        ...
        if (isLoteSefaz) {
          // Envia lote para /fd/add/sefaz-xml
          const itens = await this.enviarSefazXml(xmlText);
          if (Array.isArray(itens)) {
            itens.forEach(item => {
              listaGeral.push({
                arquivo: file.name,
                chave: item.chave ? this._limparChave(item.chave) : (this._extrairChaveDoXml(xmlText) || ''),
          ...
          const chaveExtraida = this._extrairChaveDoXml(xmlText);
          // Envia XML individual para /fd/add/xml (GRÁTIS)
          try {
            const resp = await this.enviarXmlIndividual(xmlText);
            const chave = (resp && resp.chave) ? this._limparChave(resp.chave) : (chaveExtraida || '');
            listaGeral.push({
              arquivo: file.name,
              chave: chave,
            ...
            // Se já foi cadastrada anteriormente ou erro na API mas temos a chave válida:
            if (chaveExtraida) {
              listaGeral.push({
                arquivo: file.name,
                chave: chaveExtraida,
          ...
          // Tenta extrair chave diretamente do conteúdo XML
          const chaveExtraida = this._extrairChaveDoXml(xmlText);
          if (chaveExtraida) {
            listaGeral.push({
              arquivo: file.name,
    ...
    listaGeral.forEach(item => {
      if (item.status === 'OK' && item.chave && this._validarChave(item.chave)) {
        this._addToCache({ chave: item.chave, status: 'OK', response: item });
      }
    });
    ...
    resultDiv.innerHTML = `
      ...
      <div style="overflow-x:auto;">
        <table class="data-table">
          ...
          <tbody>
            ${listaGeral.map(item => {
              const chave = this._limparChave(item.chave || '');
              const st = this._statusLabel(item.status);
              return `
              <tr>
                <td>
                  ${chave ? `<code style="font-size:.7rem;color:var(--text2);word-break:break-all;">${this._fmtChave(chave)}</code>` : `<span style="font-size:.78rem;color:var(--text3);">${item.arquivo || '—'}</span>`}
