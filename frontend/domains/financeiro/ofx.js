...
const OFX = {
  ...
  _parseOFX(content) {
    ...
    const extractTrnFromBlock = (block) => {
      const get = (tag, b) => {
    ...
    while ((m = trnRegex.exec(content)) !== null) {
      transacoes.push(extractTrnFromBlock(m[1]));
    }

    // Fallback SGML: se não encontrou tags de fechamento </STMTTRN> (comum em bancos brasileiros)
    if (transacoes.length === 0 && /<STMTTRN>/i.test(content)) {
      const parts = content.split(/<STMTTRN>/i).slice(1);
      for (const part of parts) {
        const block = part.split(/<\/BANKTRANLIST>|<\/STMTRS>|<BANKTRANLIST>/i)[0];
        if (block && /<TRNAMT>/i.test(block)) {
          transacoes.push(extractTrnFromBlock(block));
        }
      }
    }
  ...
  _avaliarMatch(trn, lan, tolValor, tolDias) {
    ...
    let diffDias = 0;
    if (dLanStr && trn.data) {
      const tLan = Date.parse(dLanStr.length === 10 ? `${dLanStr}T12:00:00` : dLanStr);
      const tTrn = Date.parse(trn.data.length === 10 ? `${trn.data}T12:00:00` : trn.data);
      if (Number.isFinite(tLan) && Number.isFinite(tTrn)) {
        diffDias = Math.abs(tLan - tTrn) / 86400000;
      }
    }

    // Se ultrapassar a tolerância de dias ou for inválido, não combina
    if (!Number.isFinite(diffDias) || diffDias > tolDias) return null;
  ...
  _aplicarBaixaConciliacao(lanId, trnData, contaBancaria) {
    ...
    if (l.tipo === 'despesa') {
      if (l.status === 'a_pagar' || !l.status) {
        patch.status = 'pago';
        if (!l.data_pagamento) patch.data_pagamento = trnData || Utils.today();
      }
    } else if (l.tipo === 'receita') {
      if (l.status === 'a_receber' || !l.status) {
        patch.status = 'recebido';
        if (!l.data_pagamento) patch.data_pagamento = trnData || Utils.today();
      }
    }
    if (!l.conta_bancaria && contaBancaria) {
