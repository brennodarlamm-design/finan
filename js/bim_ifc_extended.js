...
const BIMIFCExtendedImporter = (function() {
  ...
  function parse(text){
    ...
    const pointList3D=id=>{
      const e=entities.get(id); return e?.type==='IFCCARTESIANPOINTLIST3D'?pointListAny(id):[];
    };
    ...
    const sampleArc3D=(p1,p2,p3)=>{
      ...
      const c1=cross(b,axb),c2=cross(axb,a);
      const center=add(p1,mul(add(mul(c1,aa),mul(c2,bb)),1/den));
      ...
      const mid=angle(p2),end=angle(p3);
      const normPos=x=>{let y=x%(Math.PI*2);if(y<0)y+=Math.PI*2;return y;};
      const midPos=normPos(mid),endPos=normPos(end);
    ...
    const indexedPolyCurvePoints=e=>{
      ...
      if(!segText||segText==='    const sweptDiskSolid=(id,basis)=>{
...
if (typeof window !== 'undefined') window.BIMIFCExtendedImporter = BIMIFCExtendedImporter;
      ...
      while((m=re.exec(segText))){
        ...
        if(m[1].toUpperCase()==='LINE'){
          appendPoints(out,idx.map(i=>pts[i]).filter(Boolean));
        }else if(idx.length===3&&idx.every(i=>pts[i])){
          appendPoints(out,sampleArc3D(pts[idx[0]],pts[idx[1]],pts[idx[2]]));
          exactCurveKinds.add('indexed-arc');
    ...
    const curvePoints=(id,depth=0)=>{
      ...
      if(e.type==='IFCCIRCLE'||e.type==='IFCELLIPSE'){
        exactCurveKinds.add(e.type==='IFCCIRCLE'?'circle':'ellipse');
        return sampleConic(e,0,Math.PI*2,true);
      }
      if(e.type==='IFCTRIMMEDCURVE'){
        const base=entities.get(refOf(e.args[0]));
        if(!base||!['IFCCIRCLE','IFCELLIPSE'].includes(base.type)){partialCurveIds.add(e.id);return [];}
        const a0=trimAngle(e.args[1],base),a1=trimAngle(e.args[2],base);
        if(a0===null||a1===null){partialCurveIds.add(e.id);return [];}
        const sense=/\.T\./i.test(String(e.args[3]||''));
        let sweep=a1-a0;
        if(sense&&sweep<=0) sweep+=Math.PI*2;
        if(!sense&&sweep>=0) sweep-=Math.PI*2;
        exactCurveKinds.add('trimmed-conic');
        return sampleConic(base,a0,sweep,false);
      }
      if(e.type==='IFCCOMPOSITECURVESEGMENT'){
        const parent=refOf(e.args[2]),sameSense=/\.T\./i.test(String(e.args[1]||''));
        let pts=curvePoints(parent,depth+1);
        if(partialCurveIds.has(parent)) partialCurveIds.add(e.id);
        if(!sameSense) pts=[...pts].reverse();
        return pts;
      }
      if(e.type==='IFCCOMPOSITECURVE'){
        const out=[],segments=refsIn(e.args[0]);
        for(const segId of segments){
          const pts=curvePoints(segId,depth+1);
          if(!pts.length||partialCurveIds.has(segId)) partialCurveIds.add(e.id);
          appendPoints(out,pts);
        }
        if(out.length) exactCurveKinds.add('composite-curve');
        return out;
      }
