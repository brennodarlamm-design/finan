...
async function configuracoesFetchWithTimeout(url, options = {}, timeoutMs = 20000) {
...
const Configuracoes = {
  ...
  async loadUsers() {
    try {
      const res = await configuracoesFetchWithTimeout('/api/users', { headers: Auth.getAuthHeaders() });
  ...
  async loadEmpresaCloud() {
    try {
      const res = await configuracoesFetchWithTimeout('/api/tenant', { headers: Auth.getAuthHeaders() });
      const data = await res.json().catch(() => ({}));
  ...
  render(obraId) {
    ...
    if (!document.getElementById('cfg-tab-styles')) {
      ...
      s.id = 'cfg-tab-styles';
      s.textContent = '.cfg-tabs{display:flex;gap:4px;border-bottom:2px solid var(--border);margin-bottom:24px;overflow-x:auto;scroll-snap-type:x proximity;scrollbar-width:thin;scrollbar-color:rgba(198,255,0,.25) transparent;-webkit-overflow-scrolling:touch;padding-bottom:4px;width:100%;box-sizing:border-box}.cfg-tabs::-webkit-scrollbar{height:4px}.cfg-tabs::-webkit-scrollbar-thumb{background:rgba(198,255,0,.25);border-radius:2px}.cfg-tab{padding:9px 13px;border:none;background:transparent;color:var(--text3);font-family:inherit;font-size:.82rem;font-weight:700;cursor:pointer;border-bottom:3px solid transparent;margin-bottom:-4px;transition:all .2s;white-space:nowrap;scroll-snap-align:start;min-height:44px;display:inline-flex;align-items:center;gap:6px;border-radius:6px 6px 0 0}.cfg-tab:hover{color:var(--text);background:rgba(255,255,255,.04)}.cfg-tab-active{color:var(--accent)!important;border-bottom:3px solid var(--accent)!important;background:rgba(198,255,0,.08)!important}.cfg-audit-mobile{display:none}.cfg-user-card,.cfg-session-card{min-width:0}.cfg-user-actions{display:flex;gap:8px;flex-wrap:wrap}.cfg-session-main{flex:1;min-width:220px}@media(max-width:1024px){.cfg-tab{padding:8px 10px;font-size:.78rem}}@media(max-width:768px){.cfg-tabs{margin-left:-14px;margin-right:-14px;padding:0 14px 4px;position:relative}.cfg-tab{padding:8px 12px;font-size:.80rem}.cfg-user-card{padding:14px!important}.cfg-user-actions{width:100%;display:grid;grid-template-columns:repeat(2,minmax(0,1fr))}.cfg-user-actions .btn{width:100%}.cfg-session-card{align-items:flex-start!important}.cfg-session-main{min-width:0;width:100%}.cfg-audit-desktop{display:none!important}.cfg-audit-mobile{display:flex;flex-direction:column;gap:10px}.cfg-audit-card{padding:14px;border:1px solid var(--border);border-radius:12px;background:rgba(255,255,255,.02)}.cfg-audit-card-head{display:flex;justify-content:space-between;gap:10px;align-items:flex-start;margin-bottom:9px}.cfg-audit-card-meta{font-size:.72rem;color:var(--text3);line-height:1.5}.cfg-audit-card .btn{margin-top:10px;width:100%}}@media(max-width:430px){.cfg-tabs{margin-left:-10px;margin-right:-10px;padding-left:10px;padding-right:10px}.cfg-user-actions{grid-template-columns:1fr}.cfg-user-card>div{align-items:flex-start!important}.cfg-session-card{display:grid!important;grid-template-columns:auto 1fr}.cfg-session-card>.btn{grid-column:1/-1;width:100%}}';
  ...
  _switch(tab) {
    ...
    if (el) {
      el.classList.add('cfg-tab-active');
      try {
  ...
  async buscarCnpj() {
    ...
    try {
      const res = await configuracoesFetchWithTimeout(`/api/cnpj?cnpj=${raw}`);
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
  ...
  async saveEmpresa(e) {
    ...
    try {
      const res = await configuracoesFetchWithTimeout('/api/tenant', { method:'PATCH', headers:Auth.getAuthHeaders(), body:JSON.stringify(empresaData) });
      const data = await res.json().catch(() => ({}));
  ...
  async saveUser(e, id) {
    ...
    if (id) body.id = id;
    try {
      const res = await configuracoesFetchWithTimeout('/api/users', { method:id?'PATCH':'POST', headers:Auth.getAuthHeaders(), body:JSON.stringify(body) });
      const data = await res.json().catch(() => ({}));
  ...
  async toggleAtivo(id, ativo) {
    try {
      const res = await configuracoesFetchWithTimeout('/api/users', { method:'PATCH', headers:Auth.getAuthHeaders(), body:JSON.stringify({ id, ativo:!ativo }) });
      const data = await res.json().catch(() => ({}));
      if (!res.ok || !data.success) {
  ...
  async saveMeuPerfil(e, id) {
    ...
    try {
      const res = await configuracoesFetchWithTimeout('/api/users', { method:'PATCH', headers:Auth.getAuthHeaders(), body:JSON.stringify(body) });
      const data = await res.json().catch(() => ({}));
  ...
  async loadAudit(reset = true) {
    ...
    try {
      const res = await configuracoesFetchWithTimeout(`/api/audit?limit=50&offset=${this._auditOffset}`, { headers: Auth.getAuthHeaders() });
      const data = await res.json().catch(() => ({}));
